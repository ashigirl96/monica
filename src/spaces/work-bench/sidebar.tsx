...
export function WorkBenchSidebar() {
  ...
  const taskRunspaces = summaries.filter((ws) => ws.kind === "task_run");
  const shellRunspaces = summaries.filter((ws) => ws.kind !== "task_run");

  function renderRunspaceGroup(items: typeof summaries, label: string) {
    if (items.length === 0) return null;
    return (
      <div className="flex flex-col gap-0.5">
        <div className="px-2.5 pb-1 pt-2 text-[10px] font-medium uppercase text-muted-foreground">
          {label}
        </div>
        {items.map((ws) => (
          <button
  ...
  return (
    <div className="flex flex-col gap-0.5">
      {renderRunspaceGroup(taskRunspaces, "Task Runs")}
      {renderRunspaceGroup(shellRunspaces, "Shells")}
    </div>
  );
}
