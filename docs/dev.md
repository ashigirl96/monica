...
| `just build`                  | release ビルド (.app のみ)                  |
| `just build-debug`            | デバッグ情報入りビルド (.app のみ)          |
| `just install-local`          | .app をビルドして /Applications/ に配置     |
| `just preview`                | フロントエンドだけプレビュー                |
| `just lint`                   | oxlint                                      |
| `just fmt` / `just fmt-check` | oxfmt（書き込み / 検査）                    |
| `just check`                  | lint + fmt-check + cargo clippy             |
| `just analyze`                | `dist/stats.html` で chunk 可視化           |
