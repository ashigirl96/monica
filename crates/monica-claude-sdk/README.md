...
Monica の Workbench で動いている対話型 Claude Code セッションを Rust から運転するための Agent Runtime（親 issue: #306）。公式 Agent Runtime やワンショット CLI は使わず、ptyd の Unix ドメインソケットに直結して PTY へ入力を注入する。desktop app は経由しない。
...
Warp との相違: Warp は Claude Code には bracketed paste を使わず生バイトを送る（bracketed paste は Codex 用戦略）。この Agent Runtime が bracketed paste を使うのは、①複数行テキストの改行が Enter として解釈され途中送信されるのを Ink の paste 検知ヒューリスティックに頼らず確実に防ぐため、②`!` / `&` の mode-switch prefix がリテラル文字扱いになり、Agent Runtime からの送信で意図せず bash mode 等へ切り替わらないため。
