...
- [x] 未配線 option の明示拒否: `validate_options` が `user` / `max_buffer_size` /
      `read_timeout_secs` / `resume_session_at` / `hooks` / `sandbox` / `mcp_servers` を InvalidConfig で弾く
      （「設定したのに効かない」の防止。配線したら個別にリストから外す）。
      `max_budget_usd` / `tools` / `betas` / `output_format` / `agents` / `setting_sources` /
      `plugins` は CLI フラグ実在を確認して配線済み
