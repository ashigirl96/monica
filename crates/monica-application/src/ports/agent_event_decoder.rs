...
/// Turns a raw agent hook payload into an agent-agnostic [`AgentSignal`]. This is the single point
/// where agent-specific event names and JSON layout are interpreted; everything downstream works
...
/// Selects the right per-agent [`AgentEventDecoder`] and exposes the opaque agent event name.
...
pub trait AgentDecoders {
    ...
    /// The opaque agent event name, recovered for logging an event the decoder declined to act
    /// on (a dropped non-blocking tool call). Keeps agent field knowledge out of the driver.
