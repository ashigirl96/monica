...
impl AgentDecoders for DefaultAgentDecoders {
    fn decode(&self, agent: Agent, raw: &[u8]) -> Result<Option<AgentSignal>> {
        decoder_for(agent).decode(raw)
    }
...
/// The [`AgentEventDecoder`] for `agent`. Used only through [`DefaultAgentDecoders`] so the
/// concrete per-agent decoders never leak past this module.
