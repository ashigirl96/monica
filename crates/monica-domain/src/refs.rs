...
pub fn parse_owner_repo(url: &str) -> Result<String, DomainError> {
    let s = url.trim();
    ...
    if parts.len() < 2 {
        return Err(DomainError::UnparseableRemote(url.to_string()));
    }
...
pub fn parse_issue_number(raw: &str) -> Result<i64, DomainError> {
    let number: i64 = raw
        .trim()
        .parse()
        .map_err(|_| DomainError::InvalidIssueNumber(raw.to_string()))?;
    if number <= 0 {
        return Err(DomainError::InvalidIssueNumber(raw.to_string()));
    }
    Ok(number)
}
...
pub fn parse_issue_ref(target: &str) -> Result<(String, i64), DomainError> {
    let (repo_part, number_part) = target
        .trim()
        .split_once('#')
        .ok_or_else(|| DomainError::MissingIssueRef(target.to_string()))?;
