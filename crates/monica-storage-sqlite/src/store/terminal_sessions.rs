...
const SESSION_COLUMNS: &str = "id, runspace_id, tab_id, kind, cwd, shell, status, agent_status, agent_wait_reason,      agent_session_id, pid, rows, cols, transcript_path, exit_code, started_at, last_seen_at,      exited_at, created_at, updated_at";
            ...
            &format!(
                "UPDATE terminal_sessions
                    SET agent_status = ?2, agent_wait_reason = ?3,
                        agent_session_id = ?4, updated_at = {SET_NOW}
                  WHERE id = ?1
                ...
                &format!(
                    "UPDATE terminal_sessions
                        SET status = ?2,
                            ...
                            agent_session_id = CASE WHEN ?6 THEN NULL ELSE agent_session_id END,
