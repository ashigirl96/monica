...
use monica_domain::{
    ...
    NewTaskRun, NewTerminalSession, Project, Provider, RawJson, RefType, TaskId, TaskKind, TaskRun,
...
#[test]
fn claude_session_insert_fails_without_a_terminal_row() {
    let mut db = SqliteStore::open_in_memory().unwrap();
    let err = db.create_claude_session(new_claude_session("uuid-1", "ts-404")).unwrap_err();
    assert!(err.to_string().contains("ts-404"), "got: {err}");
    assert!(db.list_claude_sessions().unwrap().is_empty());
}
...
fn claude_session_duplicate_id_is_rejected() {
    ...
    assert_eq!(db.list_claude_sessions().unwrap().len(), 1);
}
...
fn claude_session_ends_with_its_terminal_session_and_stamps_ended_at_once() {
    ...
    db.update_terminal_session_status(&ts.id, TerminalSessionStatus::Exited, Some(0)).unwrap();
    let ended = db.get_claude_session("uuid-1").unwrap().unwrap();
    assert_eq!(ended.status, ClaudeSessionStatus::Ended);
    let first_ended_at = ended.ended_at.clone().expect("ended_at must be stamped");

    db.update_terminal_session_status(&ts.id, TerminalSessionStatus::Lost, None).unwrap();
    let again = db.get_claude_session("uuid-1").unwrap().unwrap();
    assert_eq!(again.ended_at, Some(first_ended_at), "ended_at must be stamped only once");
}
...
fn claude_session_terminal_transition_leaves_other_mappings_alone() {
    ...
    assert_eq!(
        db.get_claude_session("uuid-doomed").unwrap().unwrap().status,
        ClaudeSessionStatus::Ended
    );
    assert_eq!(
        db.get_claude_session("uuid-survivor").unwrap().unwrap().status,
        ClaudeSessionStatus::Active
    );
}

fn observation(
    status: monica_domain::ClaudeConversationStatus,
) -> monica_application::ClaudeSessionObservation<'static> {
    monica_application::ClaudeSessionObservation {
...
#[test]
fn claude_session_signal_records_event_and_state_in_one_transaction() {
    let mut db = SqliteStore::open_in_memory().unwrap();
    let ts = db.create_terminal_session(new_shell_session(Some("agent-runtime"), None)).unwrap();
    db.mark_terminal_session_started(&ts.id, Some(1), None).unwrap();
    db.create_claude_session(new_claude_session("uuid-1", &ts.id)).unwrap();
    ...
    let updated = db
        ...
        .unwrap()
        .expect("known id must update");

    assert_eq!(
        updated.conversation_status,
        monica_domain::ClaudeConversationStatus::AwaitingUser
    );
    assert_eq!(updated.wait_reason, Some(TaskRunWaitReason::AskUserQuestion));
    assert_eq!(updated.provider_session_id.as_deref(), Some("uuid-1"));
    let events = db.list_unconsumed_claude_session_events(10).unwrap();
    assert_eq!(events.len(), 1);
    assert_eq!(events[0].claude_session_id, "uuid-1");
    assert_eq!(events[0].kind, "user_input_required");
    assert_eq!(events[0].payload_json, r#"{"hook_event_name":"PreToolUse"}"#);

    // A later resolved signal clears the wait reason.
    let resolved = db
        .record_claude_session_signal(
            "uuid-1",
            "user_input_resolved",
            "{}",
            observation(monica_domain::ClaudeConversationStatus::Thinking),
        )
        .unwrap()
        .unwrap();
    assert_eq!(resolved.wait_reason, None);
    assert_eq!(
        resolved.conversation_status,
        monica_domain::ClaudeConversationStatus::Thinking
    );
}

#[test]
fn claude_session_signal_for_unknown_id_writes_nothing() {
    let mut db = SqliteStore::open_in_memory().unwrap();
    ...
    assert!(db.list_unconsumed_claude_session_events(10).unwrap().is_empty());
}

#[test]
fn claude_session_signal_never_confirms_a_pending_launch() {
    // pending -> active is the open flow's proof that the PTY outlived the launch write;
    // a hook racing it must not fabricate that proof (the open would then kill a healthy
    // session on its own Ok(false)).
    let mut db = SqliteStore::open_in_memory().unwrap();
    let ts = db.create_terminal_session(new_shell_session(Some("agent-runtime"), None)).unwrap();
    db.mark_terminal_session_started(&ts.id, Some(1), None).unwrap();
    db.create_claude_session(new_claude_session("uuid-1", &ts.id)).unwrap();
    ...
    assert_eq!(updated.status, ClaudeSessionStatus::Pending);
    assert!(db.mark_claude_session_launched("uuid-1").unwrap(), "open flow still owns the flip");
}

#[test]
fn claude_session_signal_provider_change_resets_the_transcript_cursor() {
    let mut db = SqliteStore::open_in_memory().unwrap();
    let ts = db.create_terminal_session(new_shell_session(Some("agent-runtime"), None)).unwrap();
    db.mark_terminal_session_started(&ts.id, Some(1), None).unwrap();
    db.create_claude_session(new_claude_session("uuid-1", &ts.id)).unwrap();

    let mut stamp = observation(monica_domain::ClaudeConversationStatus::Idle);
    stamp.provider_session_id = Some("uuid-1");
    db.record_claude_session_signal("uuid-1", "session_started", "{}", stamp).unwrap();
    db.set_claude_session_jsonl_offset("uuid-1", 500).unwrap();

    // Same provider session: the cursor survives.
    let mut same = observation(monica_domain::ClaudeConversationStatus::Idle);
    same.provider_session_id = Some("uuid-1");
    let row = db
        .record_claude_session_signal("uuid-1", "turn_completed", "{}", same)
        .unwrap()
        .unwrap();
    assert_eq!(row.jsonl_offset, 500);

    // A /clear moved Claude onto a new session id: new file, cursor restarts.
    let mut moved = observation(monica_domain::ClaudeConversationStatus::Idle);
    moved.provider_session_id = Some("uuid-2");
    let row = db
        .record_claude_session_signal("uuid-1", "session_started", "{}", moved)
        .unwrap()
        .unwrap();
    assert_eq!(row.provider_session_id.as_deref(), Some("uuid-2"));
    assert_eq!(row.jsonl_offset, 0);
    assert_eq!(row.transcript_session_id(), "uuid-2");
}

#[test]
fn claude_session_signal_mark_ended_is_guarded_and_stamps_once() {
    let mut db = SqliteStore::open_in_memory().unwrap();
    let ts = db.create_terminal_session(new_shell_session(Some("agent-runtime"), None)).unwrap();
    db.mark_terminal_session_started(&ts.id, Some(1), None).unwrap();
    db.create_claude_session(new_claude_session("uuid-1", &ts.id)).unwrap();
    ...
    let ended_observation = monica_application::ClaudeSessionObservation {
    ...
    let ended = db
        .record_claude_session_signal("uuid-1", "session_ended", "{}", ended_observation)
        .unwrap()
        .unwrap();
    assert_eq!(ended.status, ClaudeSessionStatus::Ended);
    let first_ended_at = ended.ended_at.expect("ended_at stamped");

    let again = db
        .record_claude_session_signal("uuid-1", "session_ended", "{}", ended_observation)
        .unwrap()
        .unwrap();
    assert_eq!(again.ended_at, Some(first_ended_at), "ended_at must not restamp");
}

#[test]
fn claude_session_events_consume_in_id_order_with_limit() {
    let mut db = SqliteStore::open_in_memory().unwrap();
    let ts = db.create_terminal_session(new_shell_session(Some("agent-runtime"), None)).unwrap();
    db.mark_terminal_session_started(&ts.id, Some(1), None).unwrap();
    db.create_claude_session(new_claude_session("uuid-1", &ts.id)).unwrap();
    for kind in ["session_started", "prompt_submitted", "turn_completed"] {
    ...
    let first_two = db.list_unconsumed_claude_session_events(2).unwrap();
    assert_eq!(
        first_two.iter().map(|e| e.kind.as_str()).collect::<Vec<_>>(),
        vec!["session_started", "prompt_submitted"]
    );

    db.mark_claude_session_events_consumed(&[first_two[0].id, first_two[1].id]).unwrap();
    let rest = db.list_unconsumed_claude_session_events(10).unwrap();
    assert_eq!(rest.len(), 1);
    assert_eq!(rest[0].kind, "turn_completed");

    db.mark_claude_session_events_consumed(&[rest[0].id]).unwrap();
    assert!(db.list_unconsumed_claude_session_events(10).unwrap().is_empty());
    db.mark_claude_session_events_consumed(&[]).unwrap();
}

/// A session whose Claude has provably booted: launch confirmed and a first hook
/// observed (provider stamped) — the state `claim_claude_session_thinking` requires.
fn booted_claude_session(db: &mut SqliteStore, claude_session_id: &str) {
    let ts = db.create_terminal_session(new_shell_session(Some("agent-runtime"), None)).unwrap();
    db.mark_terminal_session_started(&ts.id, Some(1), None).unwrap();
    db.create_claude_session(new_claude_session(claude_session_id, &ts.id)).unwrap();
    db.mark_claude_session_launched(claude_session_id).unwrap();
    let mut stamp = observation(monica_domain::ClaudeConversationStatus::Idle);
    stamp.provider_session_id = Some("provider-1");
    db.record_claude_session_signal(claude_session_id, "session_started", "{}", stamp).unwrap();
}

#[test]
fn claude_session_claim_flips_idle_to_thinking_exactly_once() {
    let mut db = SqliteStore::open_in_memory().unwrap();
    booted_claude_session(&mut db, "uuid-1");

    assert_eq!(
        db.claim_claude_session_thinking("uuid-1").unwrap(),
        monica_application::ClaudePromptClaim::Claimed
    );
    assert_eq!(
    ...
    // The claim is the lock: a second sender must observe Busy, not win too.
    assert_eq!(
        db.claim_claude_session_thinking("uuid-1").unwrap(),
        monica_application::ClaudePromptClaim::Busy(
            monica_domain::ClaudeConversationStatus::Thinking
        )
    );
}
...
#[test]
fn claude_session_claim_reports_awaiting_user_as_busy() {
    let mut db = SqliteStore::open_in_memory().unwrap();
    booted_claude_session(&mut db, "uuid-1");
    db.record_claude_session_signal(
        "uuid-1",
        "user_input_required",
        "{}",
        observation(monica_domain::ClaudeConversationStatus::AwaitingUser),
    )
    .unwrap();
...
#[test]
fn claude_session_claim_refuses_a_pending_launch() {
    let mut db = SqliteStore::open_in_memory().unwrap();
    let ts = db.create_terminal_session(new_shell_session(Some("agent-runtime"), None)).unwrap();
    db.mark_terminal_session_started(&ts.id, Some(1), None).unwrap();
    db.create_claude_session(new_claude_session("uuid-1", &ts.id)).unwrap();
...
#[test]
fn claude_session_claim_refuses_an_active_row_without_a_hook_observation() {
    // The column default is 'idle', so an active row with no provider stamp reads idle
    // while Claude is still booting — the claim must not trust it.
    let mut db = SqliteStore::open_in_memory().unwrap();
    let ts = db.create_terminal_session(new_shell_session(Some("agent-runtime"), None)).unwrap();
    db.mark_terminal_session_started(&ts.id, Some(1), None).unwrap();
    db.create_claude_session(new_claude_session("uuid-1", &ts.id)).unwrap();
    ...
    assert_eq!(
        ...
        "a refused claim must not move the state"
    );
}

#[test]
fn claude_session_claim_reports_ended_and_missing_rows() {
    let mut db = SqliteStore::open_in_memory().unwrap();
    booted_claude_session(&mut db, "uuid-1");
    let mut ended = observation(monica_domain::ClaudeConversationStatus::Idle);
    ended.mark_ended = true;
    db.record_claude_session_signal("uuid-1", "session_ended", "{}", ended).unwrap();

    assert_eq!(
        db.claim_claude_session_thinking("uuid-1").unwrap(),
        monica_application::ClaudePromptClaim::Ended
    );
    assert_eq!(
...
#[test]
fn claude_session_release_only_undoes_a_live_claim() {
    let mut db = SqliteStore::open_in_memory().unwrap();
    booted_claude_session(&mut db, "uuid-1");

    // Nothing claimed yet: nothing to release.
    assert!(!db.release_claude_session_thinking("uuid-1").unwrap());
    ...
    // A state a hook moved on (awaiting_user) is not overwritten by a release.
    db.record_claude_session_signal(
        "uuid-1",
        "user_input_required",
        "{}",
