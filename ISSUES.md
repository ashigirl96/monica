...
`monica project init` / `set` / `list` / `show` が動き、`monica.db` の `projects` テーブルに実行環境設定を保存・読込できる。`init` は repo 内で実行すると git remote から `owner/repo` を検出し、`path` を補完しつつ `.monica/{setup.sh,prompt.md}` 雛形も生成する（DB 登録と規約ファイル生成の一本化）。
...
- [ ] `monica project init [owner/repo]` が projects を upsert する（既存は更新）。引数省略時は `git remote get-url origin` から `owner/repo` を検出し、`path` を cwd で補完する。再 init では `set` で変えた設定を保持する（`path` のみ cwd 追随）。
- [ ] `init` が `.monica/setup.sh`（実行権限付き）と `.monica/prompt.md` を雛形生成する（既存は上書きせず skip）。
...
- [ ] 保存できるフィールド: `id` / `name` / `provider` / `repo` / `path` / `default_branch` / `worktree_root` / `branch_template` / `setup_timeout_sec` / `agent_default` / `agent_permission_mode` / `hooks_claude`。`provider` / `agent_default` / `agent_permission_mode` は enum で `set` 時に検証する。（setup スクリプトは規約 `.monica/setup.sh`、初期 prompt は規約 `.monica/prompt.md` 固定のため project 設定に持たない）
- [ ] 0 件時もエラーにせず空一覧/案内を出す。

### Verification

```bash
cd <repo>                # git remote origin が owner/repo を指す
monica project init      # remote 検出 + .monica/ 生成 + 登録
...
`monica issue run MON-<id>` で、project 解決 → branch 生成 → git worktree 作成 → Run 記録 → `setting_up` → `.monica/setup.sh` 実行 → 成功で `running` / 失敗で `failed`、setup.log を保存する。
...
- Issue A（Run モデル）
- Issue B（project / branch_template / worktree）
...
- Issue E（worktree + `.monica/setup.sh` が前提）
