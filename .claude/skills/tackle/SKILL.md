...
description: >
  ...
  `feature-dev:code-architect` によるプラン審査・実装・`feature-dev:code-reviewer` による
...
- マイグレーション（`crates/monica-core/src/migrations.rs` の schema 変更）を先に入れないと、それに依存するコードが動かない
...
- `crates/monica-core`（`store.rs` / `model.rs` / `migrations.rs`）や `crates/monica-cli` のロジックを変更するときはテスト — 必須・例外なし
...
monica には `/codex`（codex-rpc）が無いので、**`feature-dev:code-architect` エージェント** にプランを審査させる。このエージェントは既存コードのパターン・規約を読んだ上で実装ブループリントの妥当性を返すので、プラン審査に向く。

`code-architect` は定義上 `model: sonnet` で固定されているが、ここは設計妥当性を問う判断重めの工程なので `model="opus"` で上書きして Opus に上げる（呼び出し引数は frontmatter より優先される）。

```
Agent(subagent_type="feature-dev:code-architect", model="opus"):
...
クリティカルな指摘が返ったらプランを直し、前回のレビュー要約を添えて再度 architect に投げ、各修正を検証させる。LGTM か残課題なしまで繰り返す。クリティカルな指摘を取り込んでから、ユーザーにプランを提示して承認を得る。
...
- `crates/monica-core` / `monica-cli` のロジックが変わったらテストを足す
- `just test` でテスト通過を確認
...
**Phase A: テストカバレッジ強制**（`crates/monica-core` を変更した場合）
...
Agent(subagent_type="feature-dev:code-reviewer"):
  ...
  - git diff main...HEAD -- crates/monica-core
  - git diff --cached -- crates/monica-core
  - git diff -- crates/monica-core
...
### architect の load-bearing な主張は鵜呑みにせず該当コードを自分で読む

Step 3 で `feature-dev:code-architect` は `file:line` を指して既存コードの性質を主張する（*「この関数は X を保証しない」*等）。その主張は **load-bearing** — 外すと提案された修正が不要・有害になる。取り込む前に該当箇所を自分で開いて確認する。正しければ反映、過剰主張なら次の投げで実コードを添えて反論し、存在しないリスクへの補償を発明しない。判断できなければプランの Open Questions に書き、実装前に検証する。

### 計画修正時は checklist を本文と同期させる

プランの checklist は実装エージェントへの拘束力ある契約。architect レビューで本文のあるセクションを直したら、**同じ edit で** checklist も直す。本文が「migration をやめた」になっているのに checklist に「migration を追加」が残ると、実装側は checklist に従って誤った設計を出荷する。「どのファイルを作る/変えるか」「どの不変条件を立てるか」を触る本文変更は、必ず checklist の差分とセットにする。動作確認・テスト一覧のセクションも同様。

### crates の責務境界を越える設計をしない

monica は `monica-core`（共有ロジック・DB・モデル）/ `monica-cli`（clap コマンド）/ `monica-app`（Tauri）の 3 crate。DB アクセスやドメインロジックは core に置き、CLI と app は core を呼ぶだけにする（`PROGRESS.md` の「共有 Rust core + monica CLI、GUI は同じ core を Tauri command 経由で利用」が向かう先）。CLI に直接 SQL を書く、app に core 相当のロジックを置く、といった設計はこの方針に反するので Step 3 で必ず弾く。

### plan mode でもプラン審査を飛ばさない

plan mode の「非 readonly ツール禁止」は Step 3 の architect 審査を免除しない。審査は計画フェーズの一部で、ユーザーは `/tackle` を打った時点でそれに同意している。`ExitPlanMode` の前に architect 審査を回し、Step 3 のとおり反復する。それからユーザーにプランを見せる。
