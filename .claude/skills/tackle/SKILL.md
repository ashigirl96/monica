...
description: >
  ...
  Fable 5 subagent によるプラン審査・実装・`feature-dev:code-reviewer` による
...
### Step 0: Issue 作成（機能説明テキストが渡された場合のみ）
...
作成された Issue 番号を控え、以降の Step 1 にその番号を渡す。Step 1 は通常どおり Issue 取得 → ブランチ作成と進む。
...
- **引数で機能説明テキストが渡された場合（番号でも URL でもない自由文）:** → **Step 0: Issue 作成** に進む。
...
**Phase C: clean architecture / 依存方向レビュー**

フロント・バックエンドそれぞれの依存関係が一方向に保たれているかを専用に見るレビュアーを起動する:

```
Agent(subagent_type="feature-dev:code-reviewer"):
  Focus: clean architecture — 依存方向が内向き一方向に保たれているか。
  全差分（git diff main...HEAD / --cached / 作業ツリー）をレビュー。

  バックエンド（Rust crates）:
  - 依存は interface(cli / app) → core の一方向のみ。core が cli/app を参照していないか
  - DB アクセス・ドメインロジック・status 判定/フィルタ/入力パースは core に閉じているか。
    cli/app に直接 SQL やビジネスルールが漏れていないか
  - 型・enum・定数は Rust が single source of truth。フロント用に判定 Set / 正規表現を複製していないか

  フロント（TS / React）:
  - 依存は UI(components) → hooks/atoms(state) → commands(bindings) → backend の一方向。
    逆流（下層が上層を import、commands 層が component を知る等）がないか
  - component にビジネスルール / 判定ロジックが埋まっていないか。Rust 側で公開すべきものを再実装していないか
  - bindings.ts を迂回した型の手書き・二重定義がないか

  各指摘に file:line と「どの方向の依存が逆流しているか」を添えること。
  confidence ≥ 80 の依存違反だけを flag し、それ以外は黙ること。
```

層をまたぐ違反は確実に直す（コア層への移設・依存の向き直し）。直したらテストを再実行する。
...
Step 3 のレビュアーは `file:line` を指して既存コードの性質を主張することがある（*「この関数は X を保証しない」*等）。その主張は **load-bearing** — 外すと提案された修正が不要・有害になる。取り込む前に該当箇所を自分で開いて確認する。正しければ反映、過剰主張なら次の投げで実コードを添えて反論し、存在しないリスクへの補償を発明しない。判断できなければプランの Open Questions に書き、実装前に検証する。
