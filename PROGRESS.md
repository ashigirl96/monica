...
- 2026-05-28 #19 monica issue run --claude を実装。setup 成功後に runs/<run_id>/claude-settings.json（SessionStart/Stop/StopFailure/SessionEnd の command hook）を生成し、.monica/prompt.md を初期 prompt に claude --settings を worktree で foreground 起動、settings_path 記録・status=running（F 完了）。monica hook claude は no-op stub、receiver 本体は #20。
